...
const { getLowestRankedPosts } = useRankApi()
const userPosts = await getLowestRankedPosts('today')
...
<template>
  <UDashboardCard class="relative">
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-gray-600 dark:text-gray-300">
            Posts experiencing the most significant downward trend.
          </p>
        </div>
      </div>
    </template>
    <div class="space-y-3">
      <NuxtLink
        v-for="(
          { platform, profileId, postId, changed, total }, index
        ) in userPosts"
        :key="index"
