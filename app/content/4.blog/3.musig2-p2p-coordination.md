...
The usual answer is to trust a coordinator, run your own infrastructure, or coordinate manually. Most users can't run infrastructure. Manual coordination doesn't scale beyond small, stable groups. So a majority of people default to trusting a central coordinator, which contradicts the entire ethos of cryptocurrency.
...
MuSig2 is a multi-signature scheme where multiple parties collaboratively create a single Schnorr signature. Unlike traditional multi-sig (which puts multiple public keys in a script), MuSig2 produces an aggregated public key and a single signature that looks indistinguishable from a standard transaction.

When three people control a treasury with MuSig2, their transaction appears on-chain exactly like a single person making a payment. No visible indication of multiple signers. No way to distinguish it from someone buying coffee. This is what genuine privacy looks like: not hiding transactions, but making sophisticated ones blend seamlessly with everyday activity.

## Why MuSig2 Matters

The privacy benefit alone would justify MuSig2, but it brings efficiency and security as well.

Traditional P2SH multi-sig for three participants requires 99 bytes of public keys and roughly 210 bytes of signatures. MuSig2 with Taproot uses 33 bytes and 64 bytes respectively. That's 83% smaller, meaning lower fees, less blockchain bloat, and better scaling for the network.

The security is provably sound under the discrete logarithm assumption. MuSig2 prevents rogue key attacks through key aggregation coefficients and prevents nonce reuse through careful session management. These aren't theoretical protections; they're mathematically guaranteed properties of the scheme.

When Lotus reactivated Taproot in the 2nd Samuel epoch, we opened a path toward on-chain privacy. MuSig2 makes that privacy practical for multi-party scenarios. Complex spending conditions become indistinguishable from simple payments.

But here's the coordination problem: MuSig2 requires two rounds of communication between all participants—nonce exchange, partial signature exchange, collection, aggregation, and broadcasting. In a decentralized context, that "someone" who orchestrates all this becomes the bottleneck.
...
We started by recognizing that distributed systems had already solved similar problems: Kademlia DHT for content discovery, GossipSub for real-time pub/sub, and Byzantine consensus for agreement without trust. The question wasn't whether decentralized coordination was possible; it was how to apply these patterns to multi-party signatures.
...
Atomic swaps become cleaner. Traditional implementations rely on smart contracts or escrow services. MuSig2 enables true peer-to-peer atomic swaps with cryptographic security replacing contractual trust. Coordinator election ensures one party broadcasts while the other simply signs.
